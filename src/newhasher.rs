...
static M_MASK2: u64 = 0x8765fed1;
static M_SHIFT: i64 = 0;
type  Hash = i64;

pub fn hash_adapter<K> (value: &K) -> Hash
where
    K: ToString + Sized + std::fmt::Display,
{
    one_by_one_hash(&value.to_string())
}
...
pub fn fnv_hash(key: &str) -> u64 {
    let mut hash: u64 = 2166136261;
    let bytes = key.as_bytes();
    for v in bytes {
        hash = (hash * 16777619) ^ (*v as u64);
    }
    if M_SHIFT != 0 {
        return hash;
    }
    return (hash ^ (hash >> M_SHIFT)) & M_MASK2;
}
